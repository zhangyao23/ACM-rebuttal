...
\ans We separate new rebuttal-only evidence from submitted experiments. First, a LLaVA-family check supports the trend beyond Qwen while controlling for prompt style: both rows below use the same aggressive prompt, and only the input protocol changes from global-only to full video + retrieved clip.
\begin{center}
{\tiny
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{LLaVA-NeXT aggressive} & \textbf{Acc} & \textbf{Recall} & \textbf{FPR} \\
\midrule
Global-only baseline & 72.76\% & 43.57\% & 0.00\% \\
...
\end{center}
\tightv
Thus the gain comes from retrieved evidence under a fixed prompt, not prompt switching. Second, signed-offset diagnostics show a stable direction: non-overlapping Scout candidates fall before the GT core action, with after=0 for 4B/8B/32B. This is unlikely random drift and is consistent with pre-event contextual/precursor evidence; we do not claim causal proof or calibrated localization.

\q{Q8: What will be revised in the paper?}{QW7C,EaWG,2UeC,zyZV}
\ans We will cite all reviewer-named methods (HAWK, Holmes-VAU, Follow the Rules, VERA, VideoAgent, LITA, VadCLIP) and add a setting table separating training-free/trained, video/frame-level output, temporal annotation usage, metric, and modality. We will not mix weakly supervised frame-level SOTA with zero-shot video-level ATR as the same setting. We will also clarify Figure 2, define LAVAD $t$, separate official UCF-Crime N=290 from full-pool robustness, improve table references, and add limitations on Scout/GT alignment, FPR, and 32B over-alignment.
